...
export const FEED_QUERY = gql`
  query FeedQouery($first: Int, $skip: Int, $orderBy: LinkOrderByInput) {
...
const NEW_LINKS_SUBSCRIPTIONS = gql`
    subscription {
      newLink {
        id
        url
...
class LinkList extends Component {
  updateCacheAfterVote = (store, createVote, linkId) => {
    ...
    const orderBy = isNewPage ? 'createdAt' : null;
  ...
  subscribeToNewLinks = (subscribeToMore) => {
    subscribeToMore({
      document: NEW_LINKS_SUBSCRIPTIONS,
                ...
                <>
                  {linksToRender.map((link, index) => (
                      <Link
                          ...
                          link={link}
                          index={index}
                          updateSotreAfterVote={this.updateCacheAfterVote}
                      />
                      )
                  )}
                  {
